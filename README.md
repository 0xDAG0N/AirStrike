...
3. **Start the web interface**:
   ...
   - Verifies root privileges and exits if not running as root.
   - Creates the capture output directory configured in `web/shared.py`.
   - Starts the Socket.IO server and exposes the dashboard at `http://localhost:5000`.

4. **Open the dashboard** in a browser at `http://localhost:5000` and use the sidebar to scan networks, launch attacks, review results, or adjust settings.
